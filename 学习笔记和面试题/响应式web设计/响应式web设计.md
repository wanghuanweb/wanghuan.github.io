...
2.图片所在列利用min-width属性，能够响应父元素宽度的变化，比如增加或收缩宽度。
...
本章会关注移动设备优先的响应式设计，这里的意思是先为移动设备设计站点，然后再去适配那些有差异甚至截然不同的桌面设备。
实现web应用程序的移动版本。该章通过jQueryMobile(是一个开发移动UI元素和微件的免费开源库)优先针对移动设备优化，并针对桌面视窗进行了优化。

##### 5.1使用Safari开发者工具的用户代理设置

为了开发移动设备优先的应用，一般来说需要在本地部署并测试相关的特性。目前为止，涉及到页面设计的方法大多依赖于媒介查询，并根据客户端浏览器窗口大小进行页面优化和布局，但这不是开发工作于移动设备上的站点的唯一方法，嗅探用户代理是另一种方法。

网页页面响应式布局方法：媒介查询
移动设备响应式布局方法：媒介查询和嗅探用户代理

用户代理：用户代理信息位于http请求的头信息中，用户代理可以标识发起请求的客户端的具体信息。其中包括处理器、操作系统版本、浏览器、渲染器引擎、IP地址以及其他标识信息。
#### 6.优化响应式内容
