...
2.目标对象和观察者之间耦合关系能单独扩展和重用。
3.减少模块之间的耦合

观察者模式需要具备三个全局方法：
1.subscribe()指派函数，当特定的事件发生的时候，执行该函数
2.unsubscribe()反指派函数，当特定的事件发生的时候，该函数不执行
3.publish()按照名称发布事件，也就是依次执行事件名称相关联的所有函数。

```
//观察者模式
var observer = (function(){
    //events是对象，key是事件，value是时间需要执行的函数，是个数组
    var events = {};
